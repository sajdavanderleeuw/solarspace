...
public class BreakableWindow : MonoBehaviour {
    ...
    [Tooltip("Put here the Universe Prefab to load it into the scene when window is broken")]
    ...
    public GameObject[] breakWindow()
    {
        if (isBroken == false)
        {
            ...
            Debug.Log("window is now broken, starting async InstantiateUniverse after 3 sec waiting");
            StartCoroutine(InstantiateUniverse());

        }
        else
        {
            Debug.Log("Window is already broken, cannot break it again.");
    ...
    {
        if (useCollision == true && isBroken == false)
        {

            Debug.Log("Walked into the window, calling breakWindow();");
            breakWindow();
